...
The suite includes a deliberate second-run feed failure and proves byte-equivalent item objects survive while status records the new attempt and error.
...
- Atomic replacement is per file. Cache is committed first, then feed and status; a process interruption is recoverable on the next run without destroying last-good data.
